        ...
        if piutang.exists():
            ...
            with transaction.atomic(using=r.session["ccabang"]):
                try:
                    ...
                    if not pgw.exists():
                        return JsonResponse({"status":"error","msg":"Invalid Pegawai"},status=400)
            ...
            with transaction.atomic(using=r.session["ccabang"]):
                try:
                    ...
                    if not piutang_db.objects.using(r.session["ccabang"]).filter(pegawai_id=int(temp.pegawai_id)).exists():
                        ...
                        if jenis is None:
                            return JsonResponse({"status":'error',"msg":"Jenis saldo awal tidak ada"},status=400)
            ...
            with transaction.atomic(using=r.session["ccabang"]):
                try:
                    ...
                    if total != totalPiutang:
                        return JsonResponse({"status":'error',"msg":"Total pembayaran harus sama dengan piutang yang ada"},status=400)
        ...
        try:
            ...
            with transaction.atomic(using=r.session["ccabang"]):
                try:
                    ...
                    if jenis is None:
                        return JsonResponse({'status':"error",'msg':"Jenis transaksi 'ketemu' tidak ada"},status=400) 
                    ...
                    # Jika piutang tidak ada maka return error
                    if piutang is None:
                        return JsonResponse({"status":'error',"msg":"Piutang tidak ada"},status=400)
                    
                    # Cek jika piutang sama atau kurang dari 0 maka return error
                    if piutang.piutang <= 0:
                        return JsonResponse({"status":"error","msg":"Pegawai tersebut tidak memiliki piutang"},status=400)
                    
                    # Cek jika piutang lebih kecil dari ketemu maka return error
                    if piutang.piutang < ketemu:
                        return JsonResponse({"status":"error","msg":"Piutang lebih kecil dari ketemu"},status=400)
